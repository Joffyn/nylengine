...
mod tests {
    ...
    fn vertex_test() 
    {
        let mut verts = [TestVertex{ pos: [0f32,0f32, 0f32], color: [0f32,0f32, 0f32] }; 3];
        let rand = ||{rand::random_range(-0.5..0.5)};
        for vert in &mut verts { 
            vert.pos = [rand(),rand(),rand()];
        }
    ...
    //    let (device, queue) = &*TEST_DEVICE_CONTEXT;
    //    let memory_allocator = Arc::new(StandardMemoryAllocator::new_default(device.clone()));
    //    let image = create_image(memory_allocator, 256, 256).expect("Image failed to be created");
    //}

    //Need to create window differently in tests, for now disabled
    //
    //
    //
    //
